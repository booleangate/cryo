...
;(function() {
  ...
  function cloneWithReferences(item, references, options, savedItems, path) {
    ...
    if (CONTAINER_TYPES.indexOf(type) !== -1) {
      ...
      if (referenceIndex === -1) {
        ...
        if (typeof item === 'function' && references[referenceIndex].value.indexOf('[native code]') !== -1) {
          var error = new ReferenceError('Could not serialize native function `' + item.name + '` found at root.' + path.join('.'));
          error._keyPath = path;
          throw error;
        }
        ...
        for (var key in item) {
          if (options.isSerializable(item, key)) {
            clone[key] = cloneWithReferences(item[key], references, options, savedItems, path.concat(key));
          }
        }
      }
